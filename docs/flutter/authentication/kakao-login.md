...
> **중요 (Play Store 배포 시)**  
> Google Play Store에 업로드하면(특히 **Play App Signing**을 사용하는 경우) **스토어에 올라간 앱의 서명 인증서**가 로컬(디버그/릴리즈 키스토어)과 **달라질 수 있습니다.**  
> 그러면 디버그에서는 잘 되다가, Play Store 업로드 후에 **카카오 로그인(소셜 로그인)이 실패**할 수 있어요.  
> 이 경우 **Kakao Developers 콘솔의 Android 플랫폼 설정**에 **Play Console의 “앱 서명 키(App signing key)”로 계산한 키 해시**도 **추가로 등록**해야 합니다.
...
- **(Play Store 배포 시) Play App Signing으로 서명된 앱의 키 해시도 등록**되어 있는지 (디버그에서 되다가 업로드 후 실패하는 경우 흔함)
